...
async function fetchRSS(source) {
  ...
  try {
    ...
    for (const item of items.slice(0, 3)) {
      ...
      const descRaw = item.description?.[0] || item.summary?.[0] || item.content?.[0]?._ || '';
...
async function main() {
  ...
  const results = await Promise.allSettled(
    NEWS_SOURCES.map(source => fetchRSS(source).then(articles => ({ source, articles })))
  );
  
  for (const result of results) {
    if (result.status === 'fulfilled' && result.value.articles.length > 0) {
      allNews.push(...result.value.articles);
      fetchStatus.success.push(result.value.source.name);
    } else {
      const sourceName = result.reason?.source?.name || 'Unknown';
      fetchStatus.failed.push(sourceName);
    }
  }
  ...
  if (fetchStatus.failed.length > 0) {
